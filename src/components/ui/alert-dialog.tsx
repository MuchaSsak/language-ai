    ...
    <FullWindowOverlay>
      ...
      >
        <NativeOnlyAnimatedView
          ...
          className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center p-4"
        >
          {children}
        </NativeOnlyAnimatedView>
      </AlertDialogPrimitive.Overlay>
    ...
    <AlertDialogPortal hostName={portalHost}>
      <AlertDialogOverlay>
        <AlertDialogPrimitive.Content
          className={cn(
            "bg-secondary border-border z-50 flex w-full flex-col gap-4 rounded-lg border p-6 shadow-lg shadow-black/25",
            Platform.select({
...
function AlertDialogHeader({ className, ...props }: ViewProps) {
  return (
    <TextClassContext.Provider value="text-left sm:text-left">
...
function AlertDialogCancel({
  ...
  return (
    <TextClassContext.Provider
      value={buttonTextVariants({ className, variant: "accent" })}
    >
